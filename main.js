...
const CONFIG = {
    ...
    // DELAY SETTINGS - disesuaikan dengan loading website
    DELAYS: {
        INITIAL_LOAD: 15000,        // 15 detik setelah load page
        AFTER_CONNECT: 10000,       // 10 detik setelah connect wallet
        BETWEEN_ACTIONS: 12000,     // 12 detik antar action
        BEFORE_VERIFY: 15000,       // 15 detik sebelum verify
        RETRY_WAIT: 60000,          // 1 menit kalau kena rate limit
...
class OneFootballBotAPI {
    constructor() {
        ...
        // Setup axios dengan retry dan rate limit handling
    ...
    async run() {
        try {
            ...
            // IMPORTANT: Wait for website to fully load (seperti manual)
            console.log("⏰ Waiting for website to fully load...");
            await this.delay(CONFIG.DELAYS.INITIAL_LOAD, true);
            ...
            console.log("\n🔸 STEP 4: First 'Let's Go' (mimicking button click delay)");
            ...
            // Wait between actions (seperti loading button)
            console.log("⏰ Waiting for button to be ready...");
            await this.delay(CONFIG.DELAYS.BETWEEN_ACTIONS, true);
            ...
            // Longer wait before verify (button biasanya loading lebih lama)
            console.log("⏰ Waiting for Verify button to be ready...");
            await this.delay(CONFIG.DELAYS.BEFORE_VERIFY, true);

            // Step 6: Verify
            console.log("\n🔸 STEP 6: Verify");
            const verifyAction = await this.makeClaimRequest("verify", 1);
